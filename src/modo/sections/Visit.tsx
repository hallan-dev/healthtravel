...
import { MapPin, Clock, Calendar, Ticket, Phone, Mail, MessageCircle } from 'lucide-react';
...
const Visit = () => {
  ...
  return (
    <section
      id="contact"
      ref={sectionRef}
      className="relative w-full bg-[#050B14] py-32 px-8 lg:px-16"
    >
      {/* Section Header */}
      ...
      >
        {visitConfig.infoCards.map((card, i) => {
          ...
          return (
            <div key={i} className="info-card p-8 border border-white/10 hover:border-[#D4AF37]/40 transition-colors">
              {IconComponent && <IconComponent className="w-8 h-8 text-[#D4AF37]/80 mb-6" strokeWidth={1.5} />}
      ...
      {visitConfig.ctaText && (
        <div className="max-w-7xl mx-auto mt-16 text-center">
          <a
            href={visitConfig.ctaHref}
            target="_blank"
            rel="noreferrer"
            data-cursor="hover"
            className="inline-flex items-center gap-3 px-10 py-4 bg-[#D4AF37] text-[#050B14] museo-label hover:bg-[#E8C766] transition-colors"
          >
            {visitConfig.ctaText}
          </a>
        </div>
