    ...
    >
      ...
      <div ref={gridRef} className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-5 lg:gap-6">
        {exhibitionsConfig.exhibitions.map((exhibit) => (
          ...
          >
            ...
            {/* Hover border */}
            <div className="absolute inset-0 border border-white/0 group-hover:border-[#D4AF37]/50 transition-colors duration-500 pointer-events-none" />
          </Link>
        ))}
      </div>
      ...
      {exhibitionsConfig.ctaText && (
        <div ref={ctaRef} className="max-w-7xl mx-auto mt-12 flex justify-center">
          <Link
            to={exhibitionsConfig.ctaHref}
            data-cursor="hover"
            className="museo-label text-white border border-white/20 px-10 py-4 hover:bg-[#D4AF37] hover:text-[#050B14] hover:border-[#D4AF37] transition-all duration-300"
          >
            {exhibitionsConfig.ctaText}
          </Link>
        </div>
