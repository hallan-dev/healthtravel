...
const Hero = () => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="hidden md:flex items-center gap-8">
          {heroConfig.navLinks.map((link, i) => (
            <Link key={i} to={link.href} className="museo-label text-white/70 hover:text-white transition-colors">{link.label}</Link>
          ))}
          <Link to="/devis" className="museo-label text-[#0E1A2B] bg-[#D4AF37] px-4 py-2 hover:bg-[#E8C766] transition-colors">Devis gratuit</Link>
        </div>
      </nav>

      {/* Main hero content — 3-column: text | statue | text */}
      <div className="relative z-10 flex flex-col md:flex-row items-center justify-center h-full px-6 lg:px-12 pt-20 md:pt-0">
        {/* Left text block */}
        <div
          ref={leftTextRef}
          className="flex flex-col items-center md:items-end text-center md:text-right flex-1 md:pr-6 lg:pr-12 will-change-transform order-2 md:order-none"
        >
          <h1 className="museo-headline text-white text-[15vw] md:text-[9vw] lg:text-[7vw] leading-[0.85]">
            {heroConfig.brandLeft}
          </h1>
          <p className="hidden md:block museo-body text-white/60 text-sm md:text-base max-w-[240px] mt-6">
            {heroConfig.tagline}
          </p>
          <div className="hidden md:flex items-center gap-4 mt-6">
            {heroConfig.socialLinks.map((link, i) => (
              <a key={i} href={link.href} target="_blank" rel="noreferrer" className="museo-label text-white/40 hover:text-white transition-colors text-[10px]" data-cursor="hover">{link.label}</a>
            ))}
          </div>
        </div>
        ...
        <div
          ref={statueRef}
          className="relative flex-shrink-0 w-[64vw] md:w-[30vw] lg:w-[26vw] max-w-[480px] will-change-transform order-1 md:order-none"
        >
          {/* Badge above statue */}
          <div
            ref={badgeRef}
            className="absolute -top-10 left-1/2 -translate-x-1/2 museo-label text-[#D4AF37] text-[10px] whitespace-nowrap will-change-transform"
          >
            {heroConfig.badge}
          </div>
          <img
        ...
        <div
          ref={rightTextRef}
          className="flex flex-col items-center md:items-start text-center md:text-left flex-1 md:pl-6 lg:pl-12 will-change-transform order-3 md:order-none"
        >
          <h1 className="museo-headline text-white text-[15vw] md:text-[9vw] lg:text-[7vw] leading-[0.85]">
            {heroConfig.brandRight}
          </h1>
          <p className="hidden md:block museo-label text-white/40 mt-6 text-[10px]">{heroConfig.since}</p>
          {heroConfig.email && (
            <a
      ...
      >
        <p className="hidden sm:block museo-label text-white/30 text-[10px]">{heroConfig.scrollText}</p>
        <p className="museo-label text-white/30 text-[10px] pr-16 sm:pr-0">{heroConfig.copyrightText}</p>
      </div>
    </section>
  );
};
